...
class LoRAStackLinear(nn.Module):
    ...
    def get_ella_penalty(self):
        if self.merged_delta.shape[0] == 0 or torch.all(self.merged_delta == 0):
            return torch.tensor(0.0, device=self.A.device, dtype=self.A.dtype)

        s2 = self.scaling ** 2
        
        M1 = torch.matmul(self.B.T, self.B)
        
        Z = torch.matmul(self.A, self.merged_delta.T)
        
        M2 = torch.matmul(Z, Z.T)
        
        penalty = s2 * torch.sum(M1 * M2.T) 
...
def get_total_ella_penalty(model):
    total_penalty = 0.0
    for m in model.modules():
        if isinstance(m, LoRAStackLinear):
            total_penalty += m.get_ella_penalty()
